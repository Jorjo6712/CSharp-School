                    ...
                    case ConsoleKey.D6:
                        try
                        {
                            while (true)
                            {
                                ...
                                Console.WriteLine("Tast ind E-Mail addresse:");

                                string? email = Console.ReadLine();

                                char delimiter = '@';

                                string[] del = email.Split(delimiter);

                                string kontonavn = del[0];

                                string domain = del[1];

                                string[] domainDel = domain.Split('.');

                                string tld = domainDel[^1];



                                int minKontonavn = 1;
                                int minOrganization = 5;
                                int minDomain = 2;
                                int maxDomain = 4;

                                if (kontonavn.Length < minKontonavn || domain.Length < minOrganization || tld.Length < minDomain || tld.Length > maxDomain)
                                {
                    ...
                    case ConsoleKey.D7:
                        try
                        {
                            while (true)
                            {
                                ...
                                if (!Regex.IsMatch(email2, regexEmail))
                                {
