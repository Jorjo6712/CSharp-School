namespace Mozart
{
    class Program
    {
        ...
        static void Controller()
        {
            ...
            int minuetIndex = dice.Next(1, 7);
            int trioIndex = dice.Next(1, 7);

            int diceSum = minuetIndex + trioIndex;

            System.Media.SoundPlayer sp = new System.Media.SoundPlayer();

            for (int i = 0; i < 16; i++)
            {
                Files[i] = @"C:\temp\wav\M" + minuets[diceSum, i] + ".wav";
            }

            for (int i = 0; i < 16; i++)
            {
                Files[i] = @"C:\temp\wav\T" + trios[diceSum / 2, i] + ".wav";
            }
